    ...
    <div className="container-catalogue">
      ...
      <div className="container-products-catalogue">
        <input
          ...
          onInput={SearchWithText}
        />
        {searching === true && productosFiltrados.length === 0 ? (
          <div className="non-results">
            <p>...No hay productos que coincidan con tu busqueda</p>
            <ul>
              <li>Revisá la ortografía de la palabra.</li>
              <li>Revisá mayusculas y minusculas</li>
            </ul>
          </div>
        ) : null}

        {getPage(page).map((product) => (
      ...
      <div className="pagination">
        ...
        {pageNumber.map((number) => (
          <Link
            key={number}
            className={Number(page) === number ? "selected-page" : "page"}
            to={`/catalogo/${number}`}
          >
            {number}
          </Link>
